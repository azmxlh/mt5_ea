...
input double   AdaptiveTP_Expand_Mult   = 1.5;      // 平均の何倍以上で強トレンド判定
input double   AdaptiveTP_Squeeze_Mult  = 0.8;      // 平均の何倍以下で弱トレンド判定
...
// バンド幅が広い（エクスパンション）→ 強トレンド → 利確%を大きく（伸ばす）
// バンド幅が狭い（スクイーズ）→ 弱トレンド → 利確%を小さく（早めに確定）
//+------------------------------------------------------------------+
double CalcAdaptiveTP()
{
   // ポジションを持っているペアのBBバンド幅比率の平均を算出
   double totalRatio = 0;
   ...
   if(avgRatio >= AdaptiveTP_Expand_Mult) {
      ...
      // 線形補間: Squeeze〜Expandの間をMin〜Maxで補間
      double ratio = (avgRatio - AdaptiveTP_Squeeze_Mult) / (AdaptiveTP_Expand_Mult - AdaptiveTP_Squeeze_Mult);
