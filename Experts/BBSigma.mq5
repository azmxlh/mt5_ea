...
//--- 適応型利確設定（直近勝率に応じて利確%を動的に変更）
input bool     AdaptiveTP_Enabled       = true;    // 適応型利確(有効/無効)
input double   AdaptiveTP_Max_Pct       = 20.0;     // 強トレンド時の利確%(勝率が高い時)
input double   AdaptiveTP_Min_Pct       = 5.0;      // 弱トレンド時の利確%(勝率が低い時)
input int      AdaptiveTP_TradeCount    = 20;       // 判定に使う直近取引数
input double   AdaptiveTP_WinRate_High  = 60.0;     // この勝率以上→Max利確%
input double   AdaptiveTP_WinRate_Low   = 40.0;     // この勝率以下→Min利確%
...
//+------------------------------------------------------------------+
// 適応型利確: 直近取引の勝率に応じて利確%を動的に計算
// 勝率が高い（トレンドが出ている）→ 利確%を大きく（利益を伸ばす）
// 勝率が低い（レンジ/逆行相場）→ 利確%を小さく（早めに確定）
//+------------------------------------------------------------------+
double CalcAdaptiveTP()
{
   ...
   // 履歴を直近から取得
   if(!HistorySelect(0, TimeCurrent())) return TakeProfit_Equity_Pct;

   int deals = HistoryDealsTotal();
   if(deals == 0) return TakeProfit_Equity_Pct;

   // 直近から遡ってこのEAの決済取引を集計
   for(int i = deals - 1; i >= 0 && total < AdaptiveTP_TradeCount; i--) {
      ulong ticket = HistoryDealGetTicket(i);
      if(ticket == 0) continue;

      // このEAのマジックナンバー範囲か確認
      long dealMagic = HistoryDealGetInteger(ticket, DEAL_MAGIC);
      if(dealMagic < MagicBase || dealMagic >= MagicBase + pairCount) continue;

      // 決済取引のみ（エントリーではなくイグジット）
   ...
   // 取引数が少なすぎる場合はデフォルト値を返す
   if(total < 5) return TakeProfit_Equity_Pct;

   double winRate = (double)wins / (double)total * 100.0;

   // 勝率に応じて線形補間でTP%を算出
   double tpPct;
   if(winRate >= AdaptiveTP_WinRate_High) {
      tpPct = AdaptiveTP_Max_Pct;
   } else if(winRate <= AdaptiveTP_WinRate_Low) {
      tpPct = AdaptiveTP_Min_Pct;
   } else {
      // 線形補間: Low〜Highの間を Min〜Maxで補間
      double ratio = (winRate - AdaptiveTP_WinRate_Low) / (AdaptiveTP_WinRate_High - AdaptiveTP_WinRate_Low);
      tpPct = AdaptiveTP_Min_Pct + ratio * (AdaptiveTP_Max_Pct - AdaptiveTP_Min_Pct);
   }
