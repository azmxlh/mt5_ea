...
void CheckEntry(string sym, int magic, int idx)
{
   ...
   if(CopyBuffer(handleBB[idx], 1, 0, 3, bb_upper) < 3) return;  // Upper band
   if(CopyBuffer(handleBB[idx], 2, 0, 3, bb_lower) < 3) return;  // Lower band
   ...
   // 買いエントリー条件（ブレイクアウト型）:
   // 1. 前日終値がBB上バンドを超えた（ブレイクアウト）
   // 2. 2日前は上バンド以下だった（新規ブレイク）
   // 3. 価格が200MAより上（大トレンドが上向き）
...
{
   if(posCount - 1 >= MaxPyramid) return;
