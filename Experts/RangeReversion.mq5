...
input double   Squeeze_Mult     = 1.0;          // バンド幅がこの倍率以下でスクイーズ（レンジ）判定
...
input double   RSI_Oversold     = 30.0;         // 買い:RSIがこの値以下で買い許可
input double   RSI_Overbought   = 70.0;         // 売り:RSIがこの値以上で売り許可

//--- ADXフィルター（レンジ確認: ADXが低い=レンジ）
input bool     ADXFilter_Enabled = true;        // ADXフィルター(有効/無効)
input int      ADXFilter_Period  = 14;          // ADX期間
input double   ADXFilter_Max    = 25.0;         // ADXがこの値以下ならレンジと判定（逆張りOK）
