...
input double   Squeeze_Mult     = 1.2;          // バンド幅がこの倍率以下でスクイーズ（レンジ）判定
input int      Squeeze_MinBars  = 1;            // スクイーズが最低何バー継続しているか

//--- SL/TP設定
input double   SL_Sigma         = 3.0;          // 損切りラインのσ（レンジ帯の外側）
input double   TP_Sigma         = 0.0;          // 利確ラインのσ（0=中央線、正の値=中央から反対側へのσ）
input double   SL_Extra_Pips    = 5.0;          // SLに追加するバッファ(pips)

//--- RSIフィルター（逆張り確認）
input bool     RSI_Enabled      = true;         // RSIフィルター(有効/無効)
input int      RSI_Period       = 14;           // RSI期間
input double   RSI_Oversold     = 40.0;         // 買い:RSIがこの値以下で買い許可
input double   RSI_Overbought   = 60.0;         // 売り:RSIがこの値以上で売り許可

//--- ADXフィルター（レンジ確認: ADXが低い=レンジ）
input bool     ADXFilter_Enabled = true;        // ADXフィルター(有効/無効)
input int      ADXFilter_Period  = 14;          // ADX期間
input double   ADXFilter_Max    = 30.0;         // ADXがこの値以下ならレンジと判定（逆張りOK）
...
input int      ReentryCooldown  = 2;            // 決済後の再エントリー抑制(時間)
