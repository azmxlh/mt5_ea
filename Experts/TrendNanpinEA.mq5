//+------------------------------------------------------------------+
//| TrendNanpinEA.mq5 - Trend + Averaging Down EA                  |
//| Multi-Pattern Simultaneous Operation Support (Standard Account) |
//+------------------------------------------------------------------+
...
#define MAX_PAIRS      (PATTERN_COUNT * PAIR_COUNT)  // Maximum 16 pairs

// Pattern-specific symbol definitions (Standard Account)
const string g_patternSymbols[PATTERN_COUNT][PAIR_COUNT] = {
   {"USDJPY", "GBPJPY", "AUDJPY", "EURAUD"},  // A: JPY shorts + EUR shorts
   {"NZDJPY", "CADJPY", "CHFJPY", "GBPAUD"},  // B: JPY shorts alternative pairs + GBP longs
   {"EURUSD", "GBPUSD", "AUDUSD", "USDCHF"},  // C: USD longs focus
   {"EURJPY", "USDJPY", "GBPCHF", "AUDNZD"}   // D: High interest rate currency longs
};

// Pattern-specific swap direction
...
// --- Pattern Enable/Disable (Multiple patterns can be ON simultaneously) ---
...
struct PairState {
   string   symbol;
   bool     enabled;
   bool     windingDown;     // true = pattern OFF, waiting for existing positions to close
   int      magicNumber;
   int      patternIndex;    // Which pattern this pair belongs to (0=A, 1=B, 2=C, 3=D)
...
// Check if position with specified magic number & symbol exists
...
int OnInit()
{
   ...
   if(!accountAllowed)
   {
      PrintFormat("[TrendNanpin ERROR] This account (%d) is not permitted. Please run on an authorized account.", accountNumber);
      return(INIT_FAILED);
   }
   ...
   if(Max_Nanpin < 0)
   { Print("[TrendNanpin ERROR] Max_Nanpin must be 0 or greater (0 = unlimited)"); return(INIT_PARAMETERS_INCORRECT); }
   if(MA_Period <= 0)
   { Print("[TrendNanpin ERROR] MA_Period must be a positive value"); return(INIT_PARAMETERS_INCORRECT); }

   // Pair Configuration Initialization (Register pairs from enabled patterns in order)
   g_activePairCount = 0;
      ...
      for(int p = 0; p < PAIR_COUNT; p++)
      {
         ...
         else
         {
            ...
            if(HasPositionsForMagic(magic, sym))
            {
               ...
               g_pairs[idx].lastBarTime    = 0;
               g_activePairCount++;
               PrintFormat("[TrendNanpin INFO][Pat%s][%s] Pattern OFF - Waiting for existing positions to close",
   ...
   // Error if no enabled pairs exist
   if(g_activePairCount == 0)
   { Print("[TrendNanpin ERROR] No enabled pairs found (Pattern ON or existing positions required)"); return(INIT_PARAMETERS_INCORRECT); }
   ...
   if(SinglePairMode)
   {
      ...
      // Copy found pair to index 0 and operate only 1 pair
      if(foundIdx != 0)
      {
      ...
      g_activePairCount = 1;
      PrintFormat("[TrendNanpin INFO] Single pair mode: %s only active (Pattern %s)",
   ...
   else
   {
      ...
      PrintFormat("[TrendNanpin INFO] Multi-pair mode active: %d pairs", g_activePairCount);
   }

   // Create MA indicator handles + resolve swap direction
   for(int i = 0; i < g_activePairCount; i++)
   {
      if(!g_pairs[i].enabled) continue;
      ...
      {
         PrintFormat("[TrendNanpin ERROR] Failed to create MA: %s, err=%d", g_pairs[i].symbol, GetLastError());
         return(INIT_FAILED);
      }
      GetSwapDirection(i);
   }
...
void ProcessPair(int idx)
{
   ...
   // windingDown: Pattern OFF but waiting for existing positions to close
   if(g_pairs[idx].windingDown)
   {
      // Disable if all positions closed
      if(CountPositions(idx) == 0)
      {
         g_pairs[idx].enabled = false;
         PrintFormat("[TrendNanpin INFO][Pat%s][%s] All positions closed - stopping",
                    g_patternNames[g_pairs[idx].patternIndex], g_pairs[idx].symbol);
      }
      else
      {
         // Continue averaging down (to help closing)
         if(IsNewBar(idx))
            CheckNanpin(idx);
      }
      return;
   ...
   if(existingLot > 0)
   {
      // Existing position with mismatched lot → Continue averaging down with previous lot, no new entries
      if(posCount > 0)
...
//--- IsLotMismatch ---
// Determine if initial lot of existing positions differs from current Lots setting
// If different, return existing position lot (for averaging down continuation)
// If match or no position, return 0
double GetExistingLot(int idx)
{
   ...
   // Compare initial entry lot with current setting (considering floating point error)
...
void GetSwapDirection(int idx)
{
   ...
   PrintFormat("[TrendNanpin INFO][%s] Auto-detect swap: Long=%.2f, Short=%.2f → %s",
...
bool IsTrendAligned(int idx)
{
   ...
   {
      PrintFormat("[TrendNanpin ERROR][%s] Failed to get MA: err=%d", g_pairs[idx].symbol, GetLastError());
      return false;
   }
...
void CheckEntry(int idx)
{
   ...
   if(result)
   {
      ...
      PrintFormat("[TrendNanpin INFO][Pat%s][%s] Initial entry: %s %.2f lots @ %s",
...
void CheckNanpin(int idx)
{
   ...
   // Check unrealized loss (distance from lastEntryPrice)
   ...
   if(result)
   {
      ...
      g_pairs[idx].lastEntryPrice = entryPrice;
      PrintFormat("[TrendNanpin INFO][Pat%s][%s] Averaging down #%d: %s %.2f lots @ %s",
                 g_patternNames[g_pairs[idx].patternIndex],
      ...
      PrintFormat("[TrendNanpin ERROR][%s] Averaging down order failed: err=%d", symbol, GetLastError());
}

//--- CheckNanpinWithLot ---
// Continue averaging down with previous lot when lot changes
void CheckNanpinWithLot(int idx, double baseLot)
{
   ...
   // Calculate averaging down lot based on previous lot (+1 to apply multiplier from first averaging down)
   ...
   if(result)
   {
      ...
      g_pairs[idx].nanpinCount++;
      g_pairs[idx].lastEntryPrice = entryPrice;
      PrintFormat("[TrendNanpin INFO][Pat%s][%s] Averaging down (prev lot) #%d: %s %.2f lots @ %s",
                 g_patternNames[g_pairs[idx].patternIndex],
                 symbol, g_pairs[idx].nanpinCount,
                 (g_pairs[idx].swapDirection == 1) ? "BUY" : "SELL",
                 lots, DoubleToString(entryPrice, g_pairs[idx].digits));
...
{
   // Apply multiplier from count=0 (first averaging down) (+1 to apply from first time)
...
void CloseAllPositions(int idx)
{
   ...
   for(int i = total - 1; i >= 0; i--)
   {
      ...
      if(g_trade.PositionClose(ticket))
         ...
         PrintFormat("[TrendNanpin ERROR][%s] Failed to close position: ticket=%d, err=%d", symbol, ticket, GetLastError());
   }
...
void RestoreStateFromPositions()
{
   for(int idx = 0; idx < g_activePairCount; idx++)
   {
      ...
      if(posCount > 0)
      {
         ...
         PrintFormat("[TrendNanpin INFO][Pat%s][%s] Restore state: Position count=%d, Averaging down count=%d, Latest price=%s",
                    g_patternNames[g_pairs[idx].patternIndex],
